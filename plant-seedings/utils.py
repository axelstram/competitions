...
def get_data(path, target_size=(224,224), gen=image.ImageDataGenerator(), number_of_augmentations=1):
    res = None

    for i in range(number_of_augmentations):
        batches = get_batches(path, shuffle=False, batch_size=1, class_mode=None, target_size=target_size, gen=gen)
        b = np.concatenate([batches.next() for i in range(batches.samples)])
