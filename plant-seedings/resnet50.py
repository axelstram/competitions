...
class Resnet50():
    ...
    def finetune(self, batches):
        ...
        if self.include_top:
            ...
            for layer in model.layers: layer.trainable=False
            x = AveragePooling2D((7, 7), name='avg_pool')(model.layers[-1].output)
            x = Flatten()(x)
            x = BatchNormalization()(x)
            x = Dense(1000, activation='softmax', name='fc1_1000')(x)
            x = BatchNormalization()(x)
            x = Dropout(0.5)(x)
            x = Dense(1000, activation='softmax', name='fc2_1000')(x)
            m = Dense(batches.nb_class, activation='softmax')(x)
